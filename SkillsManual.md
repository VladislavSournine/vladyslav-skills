...
- **`/vladyslav:orchestrate`** — класифікує запит і сам маршрутизує в потрібний скіл: баг → `fix-bug`, нова поведінка → `add-feature`, проект без `docs/architecture/` → `ingest`, екрани → `design-sync`/`design-page`. Маршрут озвучується одним рядком перед запуском — можна перебити.
