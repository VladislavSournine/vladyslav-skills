...
- Skills declare `Type: Architect` or `Type: Engineer (light)` in their header
...
- **MemPalace MCP server** — required by 7 skills (`add-feature`, `fix-bug`, `ingest`, `pre-release-check`, `compact-save`, `qsave`, `memory-lint`). When editing or adding skills that call `mempalace_*` tools, declare the dependency in the skill's SKILL.md and update the README "Skills that require MemPalace" list.
